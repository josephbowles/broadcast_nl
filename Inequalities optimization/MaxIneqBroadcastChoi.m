function f = MaxIneqBroadcastChoi(Ineq,rho,dims,Party,Choi,Meas)

%Optimizes broadcast inequality 'Ineq' (for state 'rho' of local dimensions 'dims') over Choi state of (initial) party 'Party' (rest is set from 'Choi' and 'Meas')
...
%'Party' is an integer, representing the (initial) party's labeling one wants to optimize over (that is, will maximize the value of 'Ineq' over Choi state Choi{Party} )
...
%inequality, f{2} the corresponding Choi state



%Initial and final dimension of the relevant party

dIn=dims(Party);

Choic=Choi{Party};

dOut=size(Choic,1)/dIn; dT=dIn*dOut;
...
for LX=1:prod(nx)
    ...
    for LA=1:prod(oa)
        ...
        pos =  vec2ind(size(It),[vx va]);
        
        
        
        S = S + It(pos)*superkron(S0,LM(:,:,va(N),vx(N)));
...
T=sdpvar(dT,dT,'hermitian','complex');


R = (T >= 0) + (PartialTrace(T,2,[dIn dOut]) == eye(dIn));



%apply the final Choi state 


rho=ApplyChoiMap(rho,T,dimsc,Party);
